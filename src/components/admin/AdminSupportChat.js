...
const AdminSupportChat = () => {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-[320px_1fr] gap-4 h-[calc(100vh-160px)] min-h-[500px]">
      <Card className="flex flex-col overflow-hidden">
        ...
        <ScrollArea className="flex-1">
          <div className="p-2 space-y-1">
            {threads.length === 0 ? (<div className="p-4 text-sm text-muted-foreground">No conversations.</div>) : threads.map((t) => (<div key={t.id} className={`rounded-md px-2 py-2 text-sm cursor-pointer hover:bg-accent ${activeId === t.id ? "bg-accent" : ""}`} onClick={() => setActiveId(t.id)}>
        ...
        {!active ? (<div className="flex-1 flex items-center justify-center">
            ...
            <div className="p-3 border-b flex items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                {active.status !== "resolved" && (<Button size="sm" variant="outline" onClick={resolve}>
            ...
            <div className="p-3 border-t">
              <div className="flex gap-2 items-end">
                <Textarea value={reply} onChange={(e) => setReply(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) {
            e.preventDefault();
            sendReply();
        } }} placeholder="Reply to the user..." rows={2} maxLength={4000} className="resize-none" disabled={sending}/>
...
const Bubble = ({ m }) => {
    if (m.role === "system")
        return <div className="text-center text-xs text-muted-foreground italic py-1">{m.content}</div>;
    const isUser = m.role === "user";
    const isAdmin = m.role === "admin";
    return (<div className={`flex gap-2 ${isAdmin ? "justify-end" : "justify-start"}`}>
      {!isAdmin && (<div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
          {isUser ? <UserRound className="h-4 w-4 text-primary"/> : <Bot className="h-4 w-4 text-primary"/>}
        </div>)}
      <div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm whitespace-pre-wrap break-words ${isAdmin ? "bg-primary text-primary-foreground" : isUser ? "bg-muted" : "bg-accent"}`}>
        <div className="text-[10px] font-semibold mb-1 opacity-70">
          {isAdmin ? "You (Admin)" : isUser ? "User" : "AI Assistant"}
        </div>
        {m.content}
      </div>
      {isAdmin && (<div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center shrink-0">
