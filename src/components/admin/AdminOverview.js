...
import { Area, AreaChart, Bar, BarChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis, PolarGrid, PolarAngleAxis, Radar, RadarChart, } from "recharts";
...
const AdminOverview = () => {
    ...
    const totals = useMemo(() => {
        ...
        const invoiceRevenue = stats?.invoices?.reduce((sum, inv) => sum + (Number(inv.amount_paid) || 0), 0) || 0;
        // If no invoices yet, fallback to order-based calculation to show historical data
        const legacyRevenue = stores.reduce((sum, s) => sum + (s.plans?.price_pkr ?? 0), 0) +
            websiteOrders.reduce((sum, o) => sum + (Number(o.amount) || 0), 0);
        ...
        return {
            ...
            pending,
            revenue,
            completion: stores.length ? Math.round((stores.filter((s) => s.status === "activated").length / stores.length) * 100) : 0,
        };
    }, [stats]);
    ...
    const perfSeries = useMemo(() => {
        ...
        const bump = (arr, key, yearOffset) => {
            arr.forEach((row) => {
                const d = new Date(row.created_at);
                const b = buckets.find((x) => x.monthIdx === d.getMonth() && x.yearIdx - yearOffset === d.getFullYear());
                if (b)
                    b[key] += 1;
            });
        };
    ...
    const radarData = useMemo(() => {
        ...
        const engagementScore = users.length
            ...
            : 0;
        const approved = feedback.filter((f) => f.approved && f.rating);
        const avgRating = approved.length
            ? approved.reduce((s, f) => s + (Number(f.rating) || 0), 0) / approved.length
            : 0;
    ...
    const isToday = (d) => today.getFullYear() === cursor.getFullYear() &&
          ...
          <div className="flex items-center justify-center py-4">
            <div className="relative h-32 w-32">
              <svg viewBox="0 0 120 120" className="h-full w-full -rotate-90">
                ...
                <circle cx="60" cy="60" r="52" className="fill-none stroke-primary transition-all" strokeWidth="8" strokeLinecap="round" strokeDasharray={`${(totals.completion / 100) * 326.7} 326.7`}/>
              </svg>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="p-5 border-border/60">
          ...
          <div className="flex gap-2 mb-3">
            <Input placeholder="Add a quick task…" value={task} onChange={(e) => setTask(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addTask()} className="h-9"/>
            <Button size="sm" onClick={addTask} className="h-9 px-3"><Plus className="h-4 w-4"/></Button>
          </div>
          <ul className="space-y-2 max-h-56 overflow-auto">
            {tasks.map((t) => (<li key={t.id} className="flex items-center gap-2 p-2 rounded-md hover:bg-secondary/50 transition-colors">
                <input type="checkbox" checked={t.done} onChange={() => setTasks((prev) => prev.map((x) => x.id === t.id ? { ...x, done: !x.done } : x))} className="h-4 w-4 accent-primary"/>
        ...
        <Card className="p-5 border-border/60">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            return (<button key={day} className={`h-8 rounded-md text-xs transition-colors ${active
          ...
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueSeries}>
                ...
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} formatter={(v) => currency(v)}/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-2 p-5 border-border/60">
          ...
          <Table>
            ...
            <TableBody>
              {(recent ?? []).map((r) => (<TableRow key={r.id} className="border-border/40">
                  <TableCell className="font-medium text-sm">{r.name}</TableCell>
                  <TableCell className="text-xs text-muted-foreground">{r.plans?.name ?? "—"}</TableCell>
                  <TableCell>
                    <Badge variant={r.status === "activated" ? "default" : "secondary"} className="text-[10px] capitalize">
                      {r.status}
                    </Badge>
                  </TableCell>
              ...
              {!recent?.length && (<TableRow><TableCell colSpan={4} className="text-center text-sm text-muted-foreground py-6">No recent orders</TableCell></TableRow>)}
            </TableBody>
          </Table>
        </Card>
...
const StatMini = ({ label, value }) => (<div className="p-3 rounded-lg bg-background/60 border border-border/60">
    <p className="text-[10px] uppercase tracking-wide text-muted-foreground">{label}</p>
    <p className="text-xl font-display font-bold text-foreground mt-1 tabular-nums">{value}</p>
  </div>);
const InsightRow = ({ label, value }) => (<div className="space-y-1">
