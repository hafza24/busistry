    ...
    const updateOrder = useMutation({
        mutationFn: async (updates) => {
            ...
            if (wordpress_url || wordpress_username || wordpress_password) {
                ...
                if (encError)
                    throw new Error(encError.message || "Failed to encrypt credentials");
                if (encData?.error)
                    throw new Error(encData.error);
            }
    ...
    return (<div className="space-y-6">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {filtered?.map((order) => (<TableRow key={order.id}>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          {selected && (<>
              ...
              <div className="grid gap-3 text-sm md:grid-cols-2">
                ...
                {selected.logo_url && (<div><strong>Logo:</strong> <img src={selected.logo_url} alt="" className="h-12 w-12 object-contain rounded mt-1"/></div>)}
                {selected.social_media_links && Object.entries(selected.social_media_links).filter(([, v]) => v).length > 0 && (<div className="md:col-span-2">
                    <strong>Social:</strong>{" "}
                    {Object.entries(selected.social_media_links).filter(([, v]) => v).map(([k, v]) => (<span key={k} className="mr-3">{k}: {v}</span>))}
                  </div>)}
                ...
                const amountDelta = typeof ocr.amount === "number" && typeof selected.amount === "number"
                    ? ocr.amount - selected.amount
                    : null;
                const tidMatch = ocr.transaction_id && selected.transaction_id
                    ? ocr.transaction_id.trim().toLowerCase() === selected.transaction_id.trim().toLowerCase()
                    : null;
                const verdictBadge = ocr.status === "match"
                ...
                return (<div className="md:col-span-2 space-y-3 rounded-lg border border-border/60 bg-muted/20 p-4">
                          ...
                          <dl className="mt-1 space-y-1">
                            ...
                            <div className="flex justify-between gap-2">
                              ...
                              <dd className="font-mono">
                                ...
                                {amountDelta != null && amountDelta !== 0 && (<span className="ml-1 text-destructive">({amountDelta > 0 ? "+" : ""}{amountDelta.toLocaleString()})</span>)}
                              </dd>
                            </div>
                            <div className="flex justify-between gap-2">
                      ...
                      {ocr.notes && (<div className="rounded border border-amber-500/30 bg-amber-500/5 p-2 text-xs text-foreground">
                          <span className="font-semibold">Notes: </span>{ocr.notes}
                        </div>)}

                      <div className="flex items-center justify-between flex-wrap gap-2">
                        {selected.screenshot_url ? (<a href={selected.screenshot_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline">
                            <ExternalLink className="h-3.5 w-3.5"/> Open uploaded receipt
                          </a>) : (<span className="text-xs text-muted-foreground">No receipt uploaded</span>)}
                        <span className="text-[10px] text-muted-foreground">
                          {ocr.scanned_at ? `Scanned ${format(new Date(ocr.scanned_at), "PP p")}` : "Not scanned yet"}
                        </span>
                      </div>

                      {ocr.raw && (<details className="rounded border border-border/60 bg-background/60 p-2 text-xs">
      ...
      <InvoiceFormDialog open={!!invoiceOrder} onOpenChange={(o) => !o && setInvoiceOrder(null)} order={invoiceOrder}/>
    </div>);
};
