...
const AdminContactMessages = () => {
    ...
    const remove = async (id) => {
        ...
        if (error)
            return toast.error("Delete failed", { description: error.message });
    ...
    return (<div className="grid lg:grid-cols-[360px_1fr] gap-4 h-[calc(100vh-8rem)]">
      {/* List */}
      <Card className="overflow-hidden">
        ...
        <ScrollArea className="h-[calc(100%-3.25rem)]">
          {loading ? (<div className="p-6 text-sm text-muted-foreground">Loading…</div>) : messages.length === 0 ? (<div className="p-6 text-sm text-muted-foreground">No messages yet.</div>) : (<ul className="divide-y divide-border">
              {messages.map((m) => (<li key={m.id}>
                  <button onClick={() => openMessage(m)} className={`w-full text-left px-4 py-3 hover:bg-muted/50 transition-colors ${active?.id === m.id ? "bg-muted" : ""}`}>
      ...
      {/* Detail */}
      <Card className="overflow-hidden">
        {active ? (<div className="flex flex-col h-full">
            ...
            <div className="px-6 py-3 border-t border-border flex flex-wrap gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => updateStatus(active.id, "replied")} disabled={active.status === "replied"}>
          ...
          </div>) : (<div className="h-full grid place-items-center text-center p-8">
            <div>
