...
const HelpChat = () => {
    ...
    useEffect(() => { if (activeId)
        inputRef.current?.focus(); }, [activeId]);
    ...
    const send = async () => {
        ...
        if (active.mode === "human") {
            ...
            const { error } = await supabase.from("chat_messages").insert({
                thread_id: active.id, role: "user", content: text,
                author_id: user.id,
            });
            if (error)
                toast.error(error.message);
    ...
    return (<>
      ...
      <div className="container max-w-6xl py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4 h-[calc(100vh-180px)] min-h-[500px]">
          ...
          <Card className="flex flex-col overflow-hidden">
            ...
            <ScrollArea className="flex-1">
              <div className="p-2 space-y-1">
                {loadingThreads ? (<div className="p-4 text-sm text-muted-foreground">Loading...</div>) : threads.length === 0 ? (<div className="p-4 text-sm text-muted-foreground">No conversations yet.</div>) : threads.map((t) => (<div key={t.id} className={`group flex items-center gap-1 rounded-md px-2 py-2 text-sm cursor-pointer hover:bg-accent ${activeId === t.id ? "bg-accent" : ""}`} onClick={() => setActiveId(t.id)}>
                    ...
                    <button className="opacity-0 group-hover:opacity-100 p-1 hover:text-destructive" onClick={(e) => { e.stopPropagation(); deleteThread(t.id); }} aria-label="Delete conversation">
          ...
          {/* Chat area */}
          <Card className="flex flex-col overflow-hidden">
            {!active ? (<div className="flex-1 flex items-center justify-center">
                ...
                <div className="p-3 border-b flex items-center justify-between gap-2">
                  ...
                  <div className="flex gap-1">
                    {active.mode === "ai" ? (<Button size="sm" variant="outline" onClick={() => switchMode("human")}>
                        <Headphones className="h-4 w-4 mr-1"/> Talk to a human
                      </Button>) : (<Button size="sm" variant="outline" onClick={() => switchMode("ai")}>
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                  {messages.length === 0 && !streaming && (<div className="text-center text-sm text-muted-foreground py-8">
                      {active.mode === "ai"
                  ...
                  {streaming && (<MessageBubble m={{ id: "streaming", thread_id: active.id, role: "assistant", content: streaming, created_at: "" }}/>)}
                  {sending && !streaming && active.mode === "ai" && (<div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            }} placeholder={active.mode === "human" ? "Message support..." : "Ask a question..."} rows={2} maxLength={4000} className="resize-none" disabled={sending}/>
...
const MessageBubble = ({ m }) => {
    if (m.role === "system") {
        return (<div className="text-center text-xs text-muted-foreground py-2 italic">{m.content}</div>);
    }
    const isUser = m.role === "user";
    const isAdmin = m.role === "admin";
    return (<div className={`flex gap-2 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser && (<div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
          {isAdmin ? <Headphones className="h-4 w-4 text-primary"/> : <Bot className="h-4 w-4 text-primary"/>}
        </div>)}
      <div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm whitespace-pre-wrap break-words ${isUser ? "bg-primary text-primary-foreground" : isAdmin ? "bg-accent" : "bg-muted"}`}>
        {!isUser && (<div className="text-[10px] font-semibold mb-1 opacity-70">
            {isAdmin ? "Support Agent" : "AI Assistant"}
          </div>)}
        {m.content}
      </div>
      {isUser && (<div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center shrink-0">
